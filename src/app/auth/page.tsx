...
import { Mail, Lock, Eye, EyeOff, Store, ArrowLeft, User, Phone, MapPin } from "lucide-react";
...
export default function AuthPage() {
  ...
  // Signup state — email only, no matric
  ...
  const flipToSignup = () => { setIsFlipped(true); setIsForgotPassword(false); setLoginError(""); setMessage(""); };
            ...
            <div className="bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-white/30 dark:border-gray-700/30 h-full overflow-y-auto">
                ...
                <form onSubmit={handleResetPassword} className="space-y-6">
                  {resetSent ? (
                    <div className="text-center py-4 space-y-5">
                      ...
                      {resetLink && (
                        <a href={resetLink} className="block w-full py-5 px-4 rounded-2xl bg-gradient-to-r from-teal-500 to-purple-600 text-white font-black text-lg text-center shadow-xl hover:from-teal-600 hover:to-purple-700 transition-all">
                          Reset My Password →
                        </a>
                      )}
                      <button type="button" onClick={() => { setIsForgotPassword(false); setResetSent(false); setResetEmail(""); setResetLink(""); setLoginError(""); }} className="w-full text-center text-purple-600 dark:text-purple-400 font-medium underline">
                        Back to Login
                      </button>
                    </div>
                  ) : (
                    <>
                      ...
                      <button type="submit" disabled={isLoading} className="w-full py-5 rounded-2xl font-black text-white text-lg shadow-xl bg-gradient-to-r from-teal-500 to-purple-600 hover:from-teal-600 hover:to-purple-700 transition-all disabled:opacity-50">
                        {isLoading ? "Getting link..." : "Get Reset Link"}
                      </button>
                      <button type="button" onClick={() => { setIsForgotPassword(false); setLoginError(""); }} className="w-full text-center text-purple-600 dark:text-purple-400 font-medium underline">
                        Back to Login
                      </button>
                    </>
                  )}
                ...
                <form onSubmit={handleLogin} className="space-y-5">
                  <div>
                    ...
                    <input type="email" value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} placeholder="you@pau.edu.ng"
                      className="w-full p-4 rounded-2xl border-2 focus:outline-none transition-all bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder:text-gray-500 dark:placeholder:text-gray-400"
                      style={{ borderColor: loginEmail ? "#7C3AED" : "#e2e8f0" }} required disabled={isLoading} />
                  </div>
                  <div className="relative">
                    ...
                    <input type={showPass ? "text" : "password"} value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} placeholder="Enter your password"
                      ...
                      style={{ borderColor: loginPassword ? "#7C3AED" : "#e2e8f0" }} required disabled={isLoading} />
                    <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-4 top-11 text-gray-500 dark:text-gray-400 hover:text-purple-600 dark:hover:text-purple-400 transition">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <button type="button" onClick={() => setIsForgotPassword(true)} className="text-sm text-purple-600 dark:text-purple-400 font-medium hover:underline">
                      Forgot password?
                    </button>
                  </div>
                  {loginError && <p className="text-sm text-red-600 dark:text-red-400 text-center -mt-2">{loginError}</p>}
              ...
              {!isForgotPassword && (
                <p className="text-center text-sm text-gray-600 dark:text-gray-400 mt-8">
                  New to StudEx?{" "}
                  <button type="button" onClick={flipToSignup} className="font-bold text-purple-600 dark:text-purple-400 underline hover:text-purple-700 dark:hover:text-purple-300 transition">
                    Create account
                  </button>
                </p>
              )}
          ...
          <div className="absolute w-full h-full" style={{ backfaceVisibility: "hidden", WebkitBackfaceVisibility: "hidden", transform: "rotateY(180deg)" }}>
            <div className="bg-white/95 dark:bg-gray-900/95 backdrop-blur-2xl rounded-3xl shadow-2xl p-8 border border-purple-100 dark:border-gray-700/30 h-full overflow-y-auto">
              ...
              <form onSubmit={handleSignupSubmit} className="space-y-5">
                {step === 1 ? (
                  <>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input type="text" name="username" value={signupForm.username} onChange={handleSignupChange} placeholder="chinedu_tech"
                    ...
                    <div>
                      ...
                      <input type="email" name="email" value={signupForm.email} onChange={handleSignupChange} placeholder="you@pau.edu.ng"
                    ...
                    <div>
                      ...
                      <input type="tel" name="phone" value={signupForm.phone} onChange={handleSignupChange} placeholder="08012345678"
                        className="w-full px-4 py-4 rounded-2xl border-2 border-purple-200 dark:border-gray-700 focus:border-purple-500 focus:outline-none transition-all text-lg font-medium bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder:text-gray-500 dark:placeholder:text-gray-400"
                        required disabled={isLoading} />
                    </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input type={showPass ? "text" : "password"} name="password" value={signupForm.password} onChange={handleSignupChange} placeholder="Create strong password"
                          ...
                          required disabled={isLoading} />
                        <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-4 top-4 text-purple-500 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300">
                          {showPass ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                        </button>
                      </div>
                    </div>
                  ...
                  <div className="space-y-8">
                    ...
                    <button type="button" onClick={generateOTP} className="text-purple-600 dark:text-purple-400 font-bold underline text-sm w-full text-center hover:text-purple-800 dark:hover:text-purple-300" disabled={isLoading}>
                      Resend Code
                    </button>
                  </div>
                )}
              ...
              <p className="text-center text-sm text-gray-600 dark:text-gray-400 mt-6 font-medium">
                ...
                <button type="button" onClick={flipToLogin} className="font-black text-purple-600 dark:text-purple-400 underline hover:text-purple-800 dark:hover:text-purple-300" disabled={isLoading}>
                  Login here
                </button>
              </p>
            </div>
