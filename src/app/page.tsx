...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    // Update document title and meta for landing page
  ...
  return (
    <>
      {/* CRITICAL SEO FIX: Structured Data (JSON-LD) */}
      <Script
      ...
      <Script
        ...
        type="application/ld+json"
        dangerouslySetInnerHTML={{
          __html: JSON.stringify(generateStructuredData.localBusiness()),
        }}
      />
      <Script
      ...
      <section className="min-h-screen bg-gradient-to-br from-teal-600 via-purple-700 to-pink-600 flex items-center justify-center relative overflow-hidden">
        ...
        <motion.div
          className="absolute inset-0 bg-gradient-to-br from-purple-500/20 via-pink-500/20 to-teal-500/20"
          animate={{
            background: [
              "linear-gradient(135deg, rgba(168, 85, 247, 0.2) 0%, rgba(236, 72, 153, 0.2) 50%, rgba(20, 184, 166, 0.2) 100%)",
              "linear-gradient(135deg, rgba(236, 72, 153, 0.2) 0%, rgba(20, 184, 166, 0.2) 50%, rgba(168, 85, 247, 0.2) 100%)",
              "linear-gradient(135deg, rgba(20, 184, 166, 0.2) 0%, rgba(168, 85, 247, 0.2) 50%, rgba(236, 72, 153, 0.2) 100%)",
            ],
          }}
          transition={{ duration: 10, repeat: Infinity, ease: "linear" }}
        />

        {/* Campus-themed pattern overlay */}
        ...
        {/* Floating Campus Elements (Books, Stars, Sparkles) */}
        <div className="absolute inset-0 pointer-events-none">
          {particlePositions.map((pos, i) => (
            <motion.div
              key={i}
              className="absolute text-2xl"
              initial={{ y: -100, x: `${pos.x}%`, opacity: 0.4, rotate: 0 }}
              animate={{
                y: "120vh",
                opacity: [0.4, 0.7, 0.4],
                rotate: [0, 360, 720],
        ...
        <div className="relative z-10 text-center px-6 max-w-5xl mx-auto">
          ...
          <motion.div
            ...
            className="inline-flex items-center gap-2 bg-gradient-to-r from-purple-500/20 to-pink-500/20 backdrop-blur-xl border-2 border-white/30 rounded-full px-8 py-4 mb-8 shadow-2xl"
          >
            <motion.div
            ...
            <span className="text-white font-black text-base tracking-wide">🇳🇬 Nigeria's #1 Campus Marketplace 🎓</span>
          </motion.div>
          ...
          >
            ...
            <motion.span
              className="block bg-gradient-to-r from-yellow-300 via-pink-300 to-teal-300 bg-clip-text text-transparent"
              animate={{
          ...
          >
            <motion.button
              ...
              whileHover={{ scale: 1.08, boxShadow: "0 25px 80px rgba(20, 184, 166, 0.6)" }}
              whileTap={{ scale: 0.95 }}
              className="group relative px-14 py-7 bg-gradient-to-r from-teal-500 to-purple-600 text-white font-black text-2xl rounded-full shadow-2xl flex items-center gap-4 overflow-hidden"
            >
              <motion.div
                className="absolute inset-0 bg-gradient-to-r from-teal-300 via-purple-400 to-pink-400 opacity-0 group-hover:opacity-40 transition-opacity duration-300"
                animate={{ x: ["-100%", "100%"] }}
                transition={{ duration: 2, repeat: Infinity, ease: "linear" }}
              />
              <span className="relative z-10">Get Started Free 🚀</span>
              <ArrowRight className="relative z-10 w-8 h-8 group-hover:translate-x-2 transition" />
            </motion.button>
            <motion.button
              onClick={() => navigate("/auth")}
              whileHover={{ scale: 1.05, borderColor: "rgba(255, 255, 255, 0.6)" }}
              whileTap={{ scale: 0.95 }}
              className="px-14 py-7 bg-white/15 backdrop-blur-2xl border-3 border-white/40 text-white font-black text-2xl rounded-full hover:bg-white/25 transition-all shadow-xl"
            >
              Sign In
            </motion.button>
          </motion.div>
      ...
      <section className="py-40 bg-gradient-to-br from-teal-600 via-purple-700 to-pink-600 relative overflow-hidden">
        ...
        <div className="text-center px-6 relative z-10">
          ...
          >
            <motion.button
              onClick={() => navigate("/auth")}
              whileHover={{ scale: 1.1, boxShadow: "0 30px 80px rgba(20, 184, 166, 0.5)" }}
              whileTap={{ scale: 0.95 }}
